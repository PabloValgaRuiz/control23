...
void iterations(const MobMatrix& T, const std::vector<Sparse<Link>>& chosenLinks, std::vector<std::vector<Result>>& results, std::vector<std::vector<Result>>& infected);
...
static constexpr size_t sizeLinks = 45; //33
// static constexpr std::array<double,sizeLinks> links_to_choose = {13000, 14000, 15000, 16000, 18000, 20000, 24000, 28000, 32000, 36000, 40000, 45000};
    ...
    Instrumentor::Get().BeginSession("Session Name");
{
    ...
    std::vector<std::vector<Result>> total_infected(sizeLinks, std::vector<Result>(nPasos, Result{}));
    ...
    file << "population" << "\t" << "links" << "\t" << "time" << "\t" << "detected" << "\t" << "infected" << "\t" << "error_detected" << "\t" << "error_infected" << "\n";
    for(int j = 0; j < nPasos; j++){
        }
    for(int l = 0; l < sizeLinks; l++){
        for(int t = 0; t < nPasos; t++){
            results[l][t].mean /= nIterations;
            results[l][t].mean2 /= nIterations;
            results[l][t].mean2 = 1.96 * std::sqrt((results[l][t].mean2 - results[l][t].mean * results[l][t].mean)/(nIterations - 1)); //95% confidence interval

            total_infected[l][t].mean /= nIterations;
            total_infected[l][t].mean2 /= nIterations;
            total_infected[l][t].mean2 = 1.96 * std::sqrt((total_infected[l][t].mean2 - total_infected[l][t].mean * total_infected[l][t].mean)/(nIterations - 1)); //95% confidence interval

            file << link_populations[l] << "\t" << vectorChosenLinks[l].Links << "\t" << t << "\t" << results[l][t].mean << "\t" << total_infected[l][t].mean << "\t" << results[l][t].mean2 << "\t" << total_infected[l][t].mean2 << "\n";
        }
    }
    file.close();
...
void iterations(const MobMatrix& T, const std::vector<Sparse<Link>>& chosenLinks, std::vector<std::vector<Result>>& results, std::vector<std::vector<Result>>& infected){
    ...
    for(int l = 0; l < chosenLinks.size(); l++){
        ...
        for(int t = 0; t < nPasos; t++){
            ...
            infected[l][t].mean += montecarlo.totalI;
            infected[l][t].mean2 += static_cast<double>(montecarlo.totalI) * montecarlo.totalI;
            
            detected_accumulated += PobInf;
            if(detected_accumulated >= THRESHOLD && !hasReachedThreshold){
                montecarlo.setLambda(beta/2);
