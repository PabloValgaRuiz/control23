...
void iterations(const MobMatrix& T, const std::vector<MobTrMatrix>& chosenLinks, std::vector<std::vector<Result>>& results,
...
void countPopulationLinks(const MobMatrix& T, const std::vector<MobTrMatrix>& chosenLinks, std::vector<std::vector<Result>>& results, std::vector<Result>& attackRate);

#define MUESTRA_MAX 200000
std::string path = "../";
std::string name = "bogota";
                        //beta_c de bogota en p=1: 1/20.6942, 1/1.78102 para areas de ZATs
static double beta_bogota = 1.0 / 1.78102;
static double p = 1.0;
static int nPasos = 300;
...
int main(){
    ...
    auto W = readWeights(T, path + "bogota/weights.txt");
    ...
    for(size_t i = 0; i < sizeLinks; ++i){
        futures.push_back(std::move(pool.enqueue([&, i]{
            constexpr double offset = 2.2;
            size_t NlcTemp = rho.size() * (i+1 + offset) / (1 * sizeLinks + offset); //Care to not choose 0 or all the links (ex: if 1000 links, take from 166 to 866)
    ...
    countPopulationLinks(T, vectorChosenLinks, results, attackRate);
    std::cout << "Population in the first links: " << attackRate[0].population_link << std::endl;
    ...
    for(double beta = 3 * beta_bogota; beta <= 5 * beta_bogota; beta += beta_diff){
        ...
        for(auto& future : futures)
            future.wait();
        for(size_t l = 0; l < vectorChosenLinks.size(); l++){
            file << beta / beta_bogota << "\t" << attackRate[l].population_link << "\t" << attackRate[l].mean / nIterations << "\t"
                 << 2 * sqrt((attackRate[l].mean2 - attackRate[l].mean*attackRate[l].mean/nIterations)/(nIterations * (nIterations - 1)))
                 << std::endl;
            std::cout << beta / beta_bogota << "\t" << attackRate[l].population_link << "\t" << attackRate[l].mean / nIterations << "\t"
                 << 2 * sqrt((attackRate[l].mean2 - attackRate[l].mean*attackRate[l].mean/nIterations)/(nIterations * (nIterations - 1)))
                 << std::endl;
        }
    }
...
void iterations(const MobMatrix& T, const std::vector<MobTrMatrix>& chosenLinks, std::vector<std::vector<Result>>& results,
    ...
    for(size_t l = 0; l < chosenLinks.size(); l++){
        ...
        for(int t = 0; t < nPasos; t++){
            ...
            for(const auto& link : chosenLinks[l])
                if(link.I != link.J)
                    for(size_t k = 0; k < link.population; k++)
            ...
            //SHUFFLE
            const std::vector<int> sample{fisherYatesShuffle(MUESTRA_MAX, MCLabels, mt)};

            //Take pieces from the sample
            int MUESTRA = MUESTRA_MAX;
            
            for(size_t k = 0; k < MUESTRA && k < sample.size(); k++)
                if(montecarlo.getEst()[sample[k]] == E || montecarlo.getEst()[sample[k]] == I){
                    montecarlo.getEst()[sample[k]] = R;
                }
            
            std::lock_guard<std::mutex> lock(resultsMutex);
            results[l][t].mean += montecarlo.totalE + montecarlo.totalI;
            results[l][t].mean2 += static_cast<double>(montecarlo.totalE + montecarlo.totalI) * (montecarlo.totalE + montecarlo.totalI);
        }
...
void countPopulationLinks(const MobMatrix& T, const std::vector<MobTrMatrix>& chosenLinks, std::vector<std::vector<Result>>& results, std::vector<Result>& attackRate){
    size_t temp;
    for(size_t i = 0; i < chosenLinks.size(); i++){
        temp = 0;
        for(const auto& link : chosenLinks[i]){
            temp += link.population;
        }
        
        for(auto& point : results[i]){
            point.population_link = temp;
        }
        attackRate[i].population_link = temp;
    }
}
...
std::pair<MobTrMatrix, std::vector<RhoMatrix>> 
    ...
    MC_DistDiaNocheF montecarlo{0, p, T};
    montecarlo.setLambda(beta_bogota);
